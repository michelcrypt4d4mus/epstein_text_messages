...
def test_custom_html_build_path():
    assert Site.custom_html_build_path(DEFAULT_HTML_DIR.joinpath('woof.html')).name == f"{CUSTOM_HTML_PREFIX}woof.html"
    assert Site.custom_html_build_path(Site.CURATED) == DEFAULT_HTML_DIR.joinpath(CUSTOM_HTML_PREFIX + 'curated.html')
...
def test_html_output_path():
    ...
    assert Site.html_output_path(Site.NAMES) == DEFAULT_HTML_DIR.joinpath(f'{NAMES_PREFIX}epstein__ghislaine.html')
