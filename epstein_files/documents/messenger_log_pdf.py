...
class MessengerLogPdf(MessengerLog):
    ...
    def _extract_messages(self) -> list[TextMessage]:
        ...
        for match in MSG_REGEX.finditer(self.text):
            ...
            sender = JEFFREY_EPSTEIN if sender.startswith('Self') else sender

            if self.file_id == 'EFTA00781689' and timestamp_str.startswith('2018-10-0') and sender in ['', 't']:
                sender = STEVE_BANNON

            text_message = TextMessagePdf(
                author=sender,
                is_id_confirmed=len(sender) > 0 and sender != STEVE_BANNON,
                text=msg,
                timestamp_str=match.group('timestamp').strip(),
            )
