...
EMAIL_SIGNATURE_REGEXES = {
    ...
    CECILE_DE_JONGH: re.compile(r"DISCLAIMER: The information contained.{,400}immediately and delete all copies\.?", re.DOTALL),
    ...
    DARREN_INDYKE: re.compile(r"DARREN\s+K\.?\s+INDYKE.{,100}?(\**\nThe\s+information\s+contained\s+in\s+this\s+communication.{,150}?)?Darren\s+K.?[\n\s]+?[Il]ndyke(,?\s+PLLC)?(.{,80}York 10065|\s+—\s+All\s+rights\s+reserved\.? ?\n\*{50,120}(\n\**)?)", re.DOTALL | re.IGNORECASE),
