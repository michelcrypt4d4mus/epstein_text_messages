...
# Char range / truncation constants
DOC_CHAR_RANGE = (0, 4_000)
EMAIL_TRUNCATE_TO = int(DOC_CHAR_RANGE[1] / 2)
...
class DocCfg(LoggingEntity):
    ...
    Attributes:
        ...
        was_removed_from_doj_site: True if this file has since been removed from the DOJ site but is archived by Jmail
        ...
        no_doublespace (bool): don't doublespace the text in this file when displaying
    ...
    url: str = ''
    url_link_text: str = ''
    was_removed_from_doj_site: bool = False
    ...
    def __post_init__(self):
        ...
        if self.is_displayed_as_img and not self.pic_cfg:
            ...
            self.pic_cfg = PicCfg(
                id=self.id,
                date=self.date,
                file_type=self.file_type if isinstance(self, PicCfg) else 'png'
            )
    ...
    def complete_description(self) -> str:
        ...
        if self.was_removed_from_doj_site:
            description = join_truthy(description, "(removed from DOJ site / only available in Jmail)")
