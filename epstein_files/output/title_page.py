...
def print_other_page_link(epstein_files: 'EpsteinFiles') -> None:
    if args._site_type == SiteType.CURATED_MOBILE:
        return
...
def _bulleted_site_link(site_type: SiteType, link: Text) -> Text:
    ...
    if args.mobile:
        if site_type == SiteType.CHRONOLOGICAL_MOBILE and args.output_chrono:
            you_are_here = YOU_ARE_HERE
        elif site_type == SiteType.CURATED:
            you_are_here = YOU_ARE_HERE

        if site_type not in SiteType.mobile_compatible_types():
            link.stylize('dim strike')
    elif site_type == args._site_type:
        you_are_here = YOU_ARE_HERE
...
def _site_directory() -> Panel:
    ...
    links_txts = [
        _bulleted_site_link(site_type, link)
        for site_type, link in SiteType.directory_links().items()
    ]
