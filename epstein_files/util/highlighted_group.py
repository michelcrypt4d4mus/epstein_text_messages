...
HIGHLIGHTED_GROUPS = [
    ...
    HighlightedGroup(
        label='journalist',
        style='bright_yellow',
        pattern=r'Alex Yablon|Arianna( Huffington)?|Bloomberg|Charlie rose|Conchita|Sarnoff|(Emily )?Michot|George Stephanopoulus|Huffington|Ingram, David|Jonathan Karl|(Katie )?Couric|(Michele\s*)?Dargan|NYT(imes)?|Politico|Sulzberger|Susan Edelman|Vick[iy] Ward|WGBH|WSJ|[-\w.]+@(bbc|independent|mailonline|mirror|thetimes)\.co\.uk',
        emailers = {
    ...
    HighlightedGroup(
        label='lawyer',
        style='purple4',
        pattern=r'Avenatti|(David\s*)?Boies|Kate Kelly|Michael J. Pike|Paul,?\s*Weiss|Steptoe|Wein(berg|garten)',
        emailers = {
    ...
    HighlightedGroup(
        ...
        style='color(24) bold',
        pattern=r'ag|(Alicia\s*)?Valle|(Lann?a\s*)?Belohlavek|CIA|FBI|IRS|(James )?Comey|(Kirk )?Blouin|((Bob|Robert) )?Mueller|(Byung\s)?Pak|(Michael\s*)?Reiter|Police Code Enforcement|Strzok|TSA|(William\s*J\.?\s*)?Zloch',
        emailers = {
