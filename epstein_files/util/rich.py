...
from epstein_files.util.constant.strings import DEFAULT, EMAIL, NA, SiteType
...
def print_other_site_link(is_header: bool = True) -> None:
    ...
    if is_header:
        print_starred_header(f"This is the Epstein {site_type.title()}s site", num_spaces=7, num_stars=17)

    other_site_type: SiteType = TEXT_MESSAGE if site_type == EMAIL else EMAIL
    other_site_msg = "there's also a site for" + (' all of' if other_site_type == EMAIL else '')
    ...
    word_count_link = link_text_obj(WORD_COUNT_URL, 'another site showing the most frequently used words in these communiques', 'dark_goldenrod')
