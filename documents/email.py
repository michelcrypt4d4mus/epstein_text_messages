...
REPLY_LINE_PATTERN = r'(On ([A-Z][a-z]{2,9},)?\s*?[A-Z][a-z]{2,9}\s*\d+,\s*\d{4},?\s*(at\s*\d+:\d+\s*(AM|PM))?,?.*(?:[ \n]+wrote:)?|-+(Forwarded|Original)\s*Message-*|Begin forwarded message:?)'
...
SENT_FROM_REGEX = re.compile(r'^([sS]ent (from|via).*(iPad|iPhone|(AT&T )?Windows ((\d+|Mobile) )?Phone|AT&T|phone|BlackBerry.*(smartphone|device|Handheld|AT&T|T- ?Mobile))\.?)', re.MULTILINE)
