#!/usr/bin/env python
# Usage: ./scripts/download_pdf.py 9 EFTA00965086
...
from epstein_files.util.helpers.string_helper import is_integer
from epstein_files.util.logging import exit_with_error, logger


if len(args.positional_args) != 2:
    exit_with_error(f"two positional args expected (ID and dataset number)")
elif not is_integer(args.positional_args[0]):
    exit_with_error(f"2nd argument should be a number, instead it's {args.positional_args[0]}")

file_id = extract_file_id(args.positional_args[1].upper())
data_set_id = int(args.positional_args[0])
