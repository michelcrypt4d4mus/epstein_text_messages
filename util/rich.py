...
def print_header():
    ...
    print_centered(f"[link=https://oversight.house.gov/release/oversight-committee-releases-additional-epstein-estate-documents/]Oversight Committee Releases Additional Epstein Estate Documents[/link]")
    print_centered(f"[link={COFFEEZILLA_ARCHIVE_URL}]Coffeezilla Archive Of Raw Epstein Materials[/link]")
    print_centered("[link=https://jmail.world]Jmail[/link] (read His Emails via Gmail interface)")
    print_centered(f"[link={COURIER_NEWSROOM_ARCHIVE_URL}]Courier Newsroom's Searchable Archive[/link]")
    print_centered("[link=https://epsteinify.com/names]epsteinify.com[/link] (raw document images)")
    print_centered("[link=https://drive.google.com/drive/folders/1hTNH5woIRio578onLGElkTWofUSWRoH_]Google Drive Raw Documents[/link]")
    console.line()
    print_centered(fr"[link={ATTRIBUTIONS_URL}]Explanation of Author Attributions[/link]", 'magenta')
    print_centered("(thanks to [dodger_blue3][link=https://x.com/ImDrinknWyn]@ImDrinknWyn[/link][/dodger_blue3] and others for attribution help)")
    print_centered(f"If you think there's an attribution error or can deanonymize an {UNKNOWN} contact @cryptadamist.", 'dim')
