...
HIGHLIGHT_PATTERNS: dict[str, str] = {
    ...
    EUROPE_COLOR: r"Le\s*Pen|Macron|(Angela )?Merk(el|le)|(Sebastian )?Kurz|(Vi(c|k)tor\s+)?Orbah?n|Ukrain(e|ian)",
...
def print_other_site_link(is_header: bool = True) -> None:
    if args.all:
        site_type = EMAIL
        other_site_type = TEXT_MESSAGE
    else:
        site_type = TEXT_MESSAGE
        other_site_type = EMAIL

    if is_header:
        site_type = wrap_in_markup_style(f"  ******* This is the Epstein {site_type.title()}s site *******  ", TITLE_STYLE)
        print_centered(site_type)

    other_site_msg = ('all ' if other_site_type == EMAIL else '') + f'{other_site_type}s site also generated by this code'
